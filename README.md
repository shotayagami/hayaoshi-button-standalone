...
```
                        ┌─────────────────────────┐
   [プレイヤーボタン×8] ──→│                         │←── [MCP23017 I2C]
   [プレイヤーランプ×8] ←──│     Raspberry Pi         │     ├ ホストボタン×7 (GPA)
                          │     Pico 2 W             │     └ XPT2046タッチ (GPB)
   [NeoPixel LED ×8] ←────│                         │
   [DFPlayer Mini] ←──────│     Wi-Fi (STA or AP)    │──→ [ILI9341 TFT 2.8"]
                          └────────┬────────────────┘
                                   │
                          ┌────────┴────────┐
                     [管理画面]         [表示画面]
...
- MCP23017 I2C GPIOエキスパンダ（ホストボタン管理、TFT/タッチ対応時必須）
- ランプ制御 (PWM輝度): 回答権者=100%点灯、待ち=約10%点灯、未押下=消灯
- WS2812B NeoPixel LEDテープ（プレイヤー色表示、GP28）
- DFPlayer Mini スピーカーモジュール（UART1経由の効果音再生）
...
├── dfplayer.py          # DFPlayer Mini音声再生制御 (UART1, GP4/GP5)
├── mcp23017.py          # MCP23017 I2C GPIOエキスパンダドライバ
├── neopixel_ctrl.py     # WS2812B NeoPixel LEDコントローラ (GP28)
...
| GP4 | 6 | DFPlayer Mini TX (UART1) | UART OUTPUT |
| GP5 | 7 | DFPlayer Mini RX (UART1) | UART INPUT |
| GP6-GP7 | 9,10 | プレイヤー7-8 ボタン | INPUT (PULL_UP) |
| GP8-GP15 | 11,12,14,15,16,17,19,20 | プレイヤー1-8 ランプ | PWM OUTPUT |
| GP16 | 21 | I2C0 SDA (MCP23017) / 正解ボタン※ | I2C / INPUT |
| GP17 | 22 | I2C0 SCL (MCP23017) / 不正解ボタン※ | I2C / INPUT |
| GP18 | 24 | SPI0 SCK (TFT) / リセットボタン※ | SPI / INPUT |
| GP19 | 25 | SPI0 MOSI (TFT) / ARMボタン※ | SPI / INPUT |
| GP20 | 26 | TFT DC / STOPボタン※ | OUTPUT / INPUT |
| GP21 | 27 | TFT RST / ジングルボタン※ | OUTPUT / INPUT |
| GP22 | 29 | TFT CS / カウントダウンボタン※ | OUTPUT / INPUT |
| GP26 | 31 | プレイヤー5 ボタン | INPUT (PULL_UP) |
| GP27 | 32 | プレイヤー6 ボタン | INPUT (PULL_UP) |
| GP28 | 34 | NeoPixel LED データ | OUTPUT |

※ GP16-22: MCP23017使用時はI2C/SPI/TFT、未使用時は直接GPIOホストボタン
...
ハードウェアSPIピンの追加不要。タッチ未搭載のILI9341モジュールでも動作する（タッチ機能のみ無効）。
...
mpremote cp mcp23017.py :mcp23017.py
mpremote cp neopixel_ctrl.py :neopixel_ctrl.py
mpremote cp ili9341.py :ili9341.py
mpremote cp display_tft.py :display_tft.py
...
- **プレイヤーボタン5-6はGP26/GP27**: GP4/GP5がDFPlayer UART1に使用されるため右側ピンに移動

### 音声

- **音声プリロード**: ページ読み込み時に全音声ファイルをブラウザメモリにキャッシュ（Blob URL）
- **DFPlayer Mini**: UART1 (GP4=TX, GP5=RX) 9600bps。起動に約1秒必要（`await asyncio.sleep(1)`）
- **音声再生の二系統**: DFPlayer（物理スピーカー）とブラウザ音声（WebAudio）を併用可能

### 周辺デバイス

- **MCP23017**: I2C0 (GP16=SDA, GP17=SCL) 400kHz。GPA=ホストボタン7個、GPB=XPT2046タッチ用
- **MCP23017未検出時**: ホストボタンはGP16-22の直接GPIO入力にフォールバック（TFT/タッチは無効）
- **NeoPixel**: WS2812B × プレイヤー数。GP28、`neopixel` モジュール使用
- **ILI9341 TFT**: SPI0 40MHz (GP18=SCK, GP19=MOSI)。MCP23017使用時のみ初期化
- **XPT2046タッチ**: MCP23017 GPBポートでビットバングSPI。1回の座標読み取りに約10ms（I2C経由のため）
